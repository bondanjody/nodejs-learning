...
test("test response route path", async () => {
  let response = await request(app).get("/products/bondan.json");
  expect(response.text).toBe("/products/bondan.json");
  ...
  response = await request(app).get("/categories/bondan.json");
